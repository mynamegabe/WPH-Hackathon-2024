      ...
      <div className="flex flex-col gap-4">
          ...
          <CardBody className={`flex flex-row gap-4 w-auto px-4 py-8`}>
            ...
            <div className="flex gap-4">
              <div className="flex flex-col gap-2 max-w-96 w-full">
                ...
                <p className="text-sm">{applicant.description}</p>
              </div>
        ...
        {!form.conversational &&
          response.map((question, index) => {
            let field = form.fields.find(
              (field) => field.id === question.field_id
            );
            return (
              <Textarea
                isReadOnly
        ...
        {form.conversational && response[0] && (
          JSON.parse(response[0]?.response).map((field, index) => {
            return (
